...
async def register(request: Request, payload: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    # Notify all admins in the org that a new member has joined and needs assignment
    try:
        ...
        for admin in admins:
            await create_notification(
                ...
                notif_type="new_member",
                message=f"New member {user.full_name or user.email} has joined. Assign them to an operator.",
                action_url="/admin/assign-members",
                action_type="assign",
                related_entity_id=user.id,
            )
    except Exception as e:
        logger.warning(f"Failed to notify admins of new member: {e}")
