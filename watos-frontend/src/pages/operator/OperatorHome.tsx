...
import { Users, AlertTriangle, LayoutGrid, Brain, TrendingUp, ClipboardCheck, ChevronRight } from 'lucide-react'
  ...
  return (
    <motion.div initial="hidden" animate="show" transition={{ staggerChildren: 0.08 }}
      ...
      <motion.div variants={itemVariants}>
        <h1 className="text-2xl md:text-3xl font-black tracking-tight text-zinc-900">Operator Dashboard</h1>
        <p className="text-sm md:text-base text-zinc-500 font-medium mt-1">Team overview, workload health, and operational controls.</p>
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {loading || wLoading ? [1,2,3,4].map(i => <Skeleton key={i} className="h-32 rounded-3xl" />) : <>
          ...
          <div
            onClick={() => navigate('/operator/board')}
          ...
          >
            <StatCard
              ...
              icon={ClipboardCheck}
              variant={pendingReview.length > 0 ? 'warning' : 'success'}
              subtitle={pendingReview.length > 0 ? 'Awaiting your review → click' : 'All clear'}
            />
          </div>
